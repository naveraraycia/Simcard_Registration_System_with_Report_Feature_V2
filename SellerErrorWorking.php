<?php
    ...
    function checkban($conn,$nso_pass){
        $sql ="SELECT sim_status, offense_count, ban_start, ban_end FROM working_registered_simusers_db
               WHERE nsonum = '$nso_pass';";
        $result = mysqli_query($conn,$sql);
        $row = mysqli_num_rows($result);
                    ...
                    if($datetoday >= $ban_end2){
                        ...
                        if($datetoday >= $ban_end3){
                            ...
                            if($datetoday > $ban_end4){
                              $updateban = "UPDATE working_registered_simusers_db SET ban_start='--', ban_end='--', sim_status='Active Status' WHERE nsonum='$nso_pass';"; 
